...
async function showWeather(city) {
    ...
    if (data.current.precipitation > 0) {
        L.marker([city.lat, city.lon],
            { icon: rainyIcon }).addTo(map)
            .bindPopup("Det regnar här i " + city.name + "!");
    }

    else if (data.current.wind_speed_10m > 6) {
        L.marker([city.lat, city.lon],
            { icon: windyIcon }).addTo(map)
            .bindPopup("Det är blåsigt här i " + city.name + "!");
    }


    else if (data.current.cloud_cover > 50) {
        L.marker([city.lat, city.lon],
            { icon: cloudyIcon }).addTo(map)
            .bindPopup("Det är molnigt här i " + city.name + "!");
    }
    else { 
        L.marker([city.lat, city.lon],
            { icon: sunnyIcon }).addTo(map)
            .bindPopup("Solen skiner här i " + city.name + "!");
    }
}
function showWeatherforCities() {
    cities.forEach(city => {
        showWeather(city);
    });
}

const element = document.getElementById("soligt");
element.addEventListener("click", showWeatherforCities);